...
- `xsmSetCustomStructures` (datapack route A): injects custom structure sets — sets stride 7 `[salt, spacing, separation, spread(0=linear/1=triangular), dim, firstEntry, entryCount]`, entries stride 2 `[id, weight]` **grouped in JSON order** (weighted dice walk depends on it). `queryRegionStructuresGrid`/`xsmGetStructureConfig` branch on ids in `[XSM_CUSTOM_STRUCT_MIN_ID(100), XSM_CUSTOM_STRUCT_MAX_ID(1000))`; custom grid = `getFeaturePos`/`getLargeStructurePos` math + `setLargeFeatureSeed` weighted first-pick, **no biome check** (known false positives). Table is mutex-guarded shared_ptr, replaced wholesale; callers must `invalidateAll()` after re-injection
